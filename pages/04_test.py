...
st.caption("키워드와 취향 입력 → AI가 동네/음식/무드 맞춘 실제 맛집 아이디어를 뽑아줘요. (실시간 크롤링 X, 실존 가게 중심)")
...
# -------------------------------------------------------------------
# 2) 수정된 SYSTEM_PROMPT (실재하는 가게 중심)
# -------------------------------------------------------------------
SYSTEM_PROMPT = """\
You are a restaurant recommendation assistant for Seoul. 
You do NOT browse the web, but you must only output restaurants that are *highly likely to exist* in reality 
(e.g., popular on KakaoMap, Naver Map, or well-known chains). 
Avoid inventing fake or imaginary names.

Return diverse, plausible, and realistic suggestions given the user's district and food keyword.
...

{
  ...
  "recommendations": [
    {
      "name": "string (Korean, must sound like a real restaurant that likely exists on Kakao/Naver Map)",
      "area_hint": "string (동/로/랜드마크 등 간단 위치 힌트)",
      "category": "예: 한식/일식/중식/아시안/바/디저트",
      "signature_menu": "대표 메뉴 1-2개",
      "price_per_person": "예: 1.5만~2.5만",
      "fit_reason": "왜 이 가게가 조건에 맞는지 (자연스러운 한국어, MZ톤)",
      "pro_tip": "주문·좌석·대기 관련 짧은 팁 (한국어)",
      "search_query": "사용자가 네이버/카카오에 직접 검색할 수 있는 한 줄 검색어"
    }
  ]
}

Rules:
- 이름은 반드시 실제 존재 가능성이 높은 가게명만 사용 (창작 금지)
- 실존 체인점(예: 이디야커피, 홍콩반점 등) 포함 가능
- 존재 여부가 불확실한 경우 유명 지역 기반 가게명 사용 (예: '홍대 ○○식당', '강남역 △△포차')
- 정확한 주소·전화번호·운영시간은 제공 금지
- 5개 이내 추천
- 서울 내 해당 구 중심으로 구성
- 한국어로 자연스럽고 재치있게 작성
"""
...
요청:
- 위 조건을 만족하는 '서울 {gu}' 중심의 실제 맛집 5곳 이내 추천
- 반드시 JSON (schema 준수)
- 각 항목에 search_query 포함 (사용자가 네이버/카카오에서 바로 검색 가능)
"""
...
with st.container():
    ...
    if clicked:
        ...
        with st.spinner("AI가 실제로 존재하는 맛집을 고르는 중…🍜"):
            try:
